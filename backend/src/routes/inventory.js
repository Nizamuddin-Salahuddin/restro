...
// Temporarily comment out adminAuth to test if this is causing import failure
...
// Setup inventory tables (admin only, run once)
router.post('/setup', async (req, res) => {
  try {
    ...
    if (checkResult.rows.length === 3) {
      return res.json({
        success: true,
        message: 'Inventory tables already exist'
      });
    }
    ...
    res.json({
      success: true,
      message: 'Inventory tables created successfully with sample data'
    });
...
// Get all inventory items with current stock (temporarily without auth)
